...
public class MyWebScoket {
	...
	public void onOpen(Session session, @PathParam("name") String name) {
		...
		String msg = "上线了" + "我的频道号是" + session.getId() + ",当前连接人数为：" + webScoketset.size();
		this.session.getAsyncRemote().sendText(json(name,msg) );
	}
...
//             socketEntity.setToUser ( toUser );//这个有客户端进行设置
	...
	private void broadcast(SocketEntity socketEntity, String name) {
		for (MyWebScoket myWebScoket : webScoketset) {
			...
			myWebScoket.session.getAsyncRemote().sendText(json(name, socketEntity.getMessage()));
		}
	}

	private String json(String name, String msg) {
		JSONObject jb = new JSONObject();
		jb.accumulate("name", name);
		jb.accumulate("msg", msg);
		return jb.toString();
	}
